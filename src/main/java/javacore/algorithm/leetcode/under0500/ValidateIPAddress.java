		...
		if (arr.length == 8) {
			for (int i = 0; i < 8; i++) {
				...
				for (int j = 0; j < arr[i].length(); j++) {
					if (!Character.isLetterOrDigit(arr[i].charAt(j)) || (Character.isLetter(arr[i].charAt(j))
							&& Character.toUpperCase(arr[i].charAt(j)) - 'F' > 0)) {
						return "Neither";
					}
				}
			}
	...
	private String checkIPv4(String queryIP) {
		...
		if (arr.length == 4) {
			for (int i = 0; i < 4; i++) {
				...
				if (x > 255 || x < 0 || (x != 0 && arr[i].charAt(0) == '0' && arr[i].length() > 1)
						|| (x == 0 && arr[i].length() > 1)) {
					return "Neither";
				}
			}
